                        ...
                        <table class="min-w-full text-left text-sm font-inter text-black">
                            <thead class="border-b border-neutral-200 font-medium ">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-4">Opening</th>
                                    <th scope="col" class="px-6 py-4">Closing</th>
                                    <th scope="col" class="px-6 py-4">Lottery Time</th>
                                    <th scope="col" class="px-6 py-4">Multiplier</th>
                                    <th scope="col" class="px-6 py-4">Twist Multiplier</th>
                                    <th scope="col" class="px-6 py-4">Closing Amount</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(setting,index) in settingList"
                                    class="border-b border-neutral-200 transition duration-300 ease-in-out hover:bg-neutral-100">
                                    <td class="whitespace-nowrap px-6 py-4 font-medium">
                                        {{ index+1 }}
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">
                                        {{ formatDate(setting.opening_date_time) }}
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">
                                        {{ formatDate(setting.closing_date_time) }}
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">
                                        {{ formatDate(setting.lottery_date_time) }}
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">
        ...
        <div data-twe-modal-dialog-ref
            ...
            <div
                ...
                <div class="relative flex-auto p-4" data-twe-modal-body-ref>
                    <div class="mb-6">
                        <label for="opening" class="text-sm mb-3 relative block">Opening Date Time</label>
                        <input type="datetime-local" v-model="openDateTime" @change="getOpenDateTime()"
                            class="block w-full py-2 px-2 border border-gray-400 text-sm rounded-md bg-white focus:ring-0 focus:shadow-none relative">
                    </div>
                    <div class="mb-6">
                        <label for="closing" class="text-sm mb-3 relative block">Closing Date Time</label>
                        <input type="datetime-local" v-model="closeDateTime" @change="getCloseDateTime()"
                            class="block w-full py-2 px-2 border border-gray-400 text-sm rounded-md bg-white focus:ring-0 focus:shadow-none relative">
                    </div>
                    <div class="mb-6">
                        <label for="multiplier" class="text-sm mb-3 relative block">Bet Multiplier</label>
                        <input type="text" v-model="betMultiplier" id="multiplier" placeholder="Multiplier"
                        class=" block w-full py-2 px-2 border border-gray-400 text-sm rounded-md bg-white focus:ring-0 focus:shadow-none">
                    </div>
                    <div class="mb-6">
                        <label for="twist_multiplier" class="text-sm mb-3 relative block">Twist Multiplier</label>
                        <input type="text" v-model="twistMultiplier" id="twist_multiplier" placeholder="Twist Multiplier"
                        class=" block w-full py-2 px-2 border border-gray-400 text-sm rounded-md bg-white focus:ring-0 focus:shadow-none">
                    </div>
                    <div class="mb-6">
                        <label for="closing_amount" class="text-sm mb-3 relative block">Closing Amount</label>
                        <input type="text" v-model="closingAmount" id="closing_amount" placeholder="Closing Amount"
                        class=" block w-full py-2 px-2 border border-gray-400 text-sm rounded-md bg-white focus:ring-0 focus:shadow-none">
                    </div>
                </div>
                <div
                    ...
                    <button type="button" @click="btnClickedAddGameSetting()"
...
export default {
    ...
    methods: {
        ...
        getOpenDateTime(){
            ...
            this.newOpenDateTime = openDate+'-'+openMonth+'-'+openYear+' '+openTime
        },
        getCloseDateTime(){
            ...
            this.newCloseDateTime = closeDate+'-'+closeMonth+'-'+closeYear+' '+closeTime
        },
